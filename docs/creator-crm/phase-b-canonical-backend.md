...
> 상태: Phase B 구현 완료 / staging 조건부 No-Go — stable slug를 포함한 1–11 자동화와 staging migration·게시 lifecycle 통과, remote purge·남은 browser acceptance 보완 필요
>
...
- [x] local 재현과 staging `0004`–`0007` migration, unique·foreign key·revision 충돌 검사 통과
- [ ] reload·새 browser tab의 draft·asset 복원은 통과; browser process 재시작·재로그인은 미확인
...
- [x] staging 비인증 Studio `302` Access 차단과 local 잘못된 same-origin write 거부
- [x] `npm run lint`와 `npm test` 통과
- [x] `technical-index.md`와 generated binding·migration 계약 동기화

## 9. 완료 기록

- commit: schema·D1 domain helper `b5d2a33`; taxonomy `e58dbfa`; asset manifest/retention `e040f31`; draft autosave `c06634e`; 안전한 작업 이동 `b013038`; Studio UI `d46f49a`; 최초 게시 알림 `a4ddae7`; update 결과 불명 복구 `0afd45f`; 공개 lifecycle·permanent purge `4ff82a9`; pin·Hero `a218acc`; archived lifecycle UI 복구 `7d72172`; stable canonical slug `5b4cf81`
- migration: `0004_phase_b_canonical_schema.sql`. Phase A 7-table row를 보존하는 outbox rebuild와 notification·cache 작업 pair, pointer·outbox post ownership, 승인본과 job-bound candidate snapshot·state·delete 및 outbox identity 불변성, active delivery 중 비검증 current·Discord mapping 이동 차단, topic·asset 상한과 같은-post asset, 단일 pin·Hero rank, lowercase SHA-256, version·asset cleanup·reverse asset·delivery query index를 추가함. fresh `0001`–`0004`와 기존 `0001`–`0003` 정상 row upgrade를 local SQLite에서 재현하고, 기존 비정상 hash·taxonomy·asset/outbox ownership row와 진행 중인 구형 Discord create/update job은 rebuild 전에 fail closed함. `PRAGMA foreign_key_check`와 representative `EXPLAIN QUERY PLAN`을 통과했고 Wrangler local migration ledger에서도 적용 뒤 재실행이 `No migrations to apply`로 끝남. staging은 2026-08-31에 pre-migration bookmark `00000018-00000000-000050d8-d96c53777adfadfdfbbad2a9d905c4dd`를 남기고 `0004`–`0006`을 적용했으며 기존 post 4·version 9·asset 7·job 16행을 보존하고 foreign key 오류 0건, 재실행 `No migrations to apply`를 확인함. production에는 미적용
...
- stable canonical slug: `worker/studio-domain.ts`가 NFC 제목을 Unicode letter·mark·number 기준으로 정규화하고 `--{post_id 앞 8자}`를 붙인 slug를 candidate/outbox와 같은 D1 batch에서 `coalesce`로 한 번만 배정함. `0007_phase_b_stable_slug.sql`은 비초안의 NULL slug와 잘못된 suffix·case·separator를 insert/update에서 거부하고, 한 번 배정된 slug 변경을 `stable_slug_immutable`로 차단함. fresh `0001`–`0007`과 재실행 `No migrations to apply`를 격리 Wrangler local DB에서 확인함. staging에는 기존 archived post 5건을 같은 규칙으로 명시적 backfill한 뒤 bookmark `0000001e-00000000-000050d8-e7f89ad0eca2db0cef26eedc8f00400e`를 남기고 `0007`을 적용했으며, foreign key 오류·비초안 NULL slug·형식 위반이 모두 0건이고 세 trigger가 존재함. production에는 미적용
...
- rollback: Wrangler migration ledger로 한 번만 적용한다. 원격 적용 뒤 `0004`–`0007`을 수정·삭제하거나 이전 migration을 재실행하지 않고, 문제가 생기면 영향 trigger·index 또는 outbox CHECK를 되돌리는 새 forward migration을 작성한다
