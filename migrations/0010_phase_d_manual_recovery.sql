-- Phase D adds operator-approved current alignment and reversible Discord
-- mapping recovery to the existing outbox without adding storage tables.
PRAGMA legacy_alter_table = ON;
ALTER TABLE delivery_jobs RENAME TO delivery_jobs_recovery_previous;
...
CREATE TABLE delivery_jobs (
  ...
  status TEXT NOT NULL CHECK (
    status IN (
      'queued', 'processing', 'retrying', 'queue_failed', 'verifying',
      'finalizing', 'succeeded', 'failed', 'outcome_unknown'
    )
  ),
  ...
  CHECK (
    (
      target = 'discord' AND action = 'taxonomy'
      AND post_id IS NULL AND version_id IS NULL AND asset_id IS NULL
      AND expected_hash IS NULL AND delivered_hash IS NULL
    ) OR (
      post_id IS NOT NULL AND (
        ...
        (
          target = 'discord' AND asset_id IS NULL AND (
            ...
            (
              action = 'check' AND version_id IS NOT NULL
              AND remote_id IS NOT NULL AND remote_aux_id IS NOT NULL
              AND expected_hash IS NULL
            ) OR
            (
              action IN ('align', 'reconnect') AND version_id IS NOT NULL
              AND remote_id IS NOT NULL AND remote_aux_id IS NOT NULL
              AND expected_hash IS NOT NULL
            ) OR
            (
              action = 'detach' AND version_id IS NOT NULL
              AND remote_id IS NOT NULL AND remote_aux_id IS NOT NULL
              AND expected_hash IS NULL
            )
          )
...
SELECT
  ...
  attempts, expected_hash, delivered_hash, error_code, last_error,
  created_at, updated_at, completed_at
FROM delivery_jobs_recovery_previous;

DROP TABLE delivery_jobs_recovery_previous;
...
WHEN (NEW.expected_hash IS NOT NULL AND (
  length(NEW.expected_hash) != 64 OR NEW.expected_hash GLOB '*[^0-9a-f]*'
)) OR (NEW.delivered_hash IS NOT NULL AND (
  length(NEW.delivered_hash) != 64 OR NEW.delivered_hash GLOB '*[^0-9a-f]*'
))
BEGIN
...
WHEN (NEW.version_id IS NOT NULL AND NOT EXISTS (
  SELECT 1 FROM studio_post_versions AS version
...
BEFORE UPDATE OF
  dedupe_key, post_id, version_id, asset_id, target, action, payload_json,
  expected_hash, created_at
ON delivery_jobs
FOR EACH ROW
...
FOR EACH ROW
WHEN OLD.target = 'discord' AND OLD.action IN ('create', 'update')
  AND EXISTS (
    SELECT 1
    FROM studio_post_versions AS version
    JOIN studio_posts AS post ON post.id = version.post_id
    WHERE version.id = OLD.version_id AND version.state = 'candidate'
