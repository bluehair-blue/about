-- Phase B taxonomy catalog and Discord Forum synchronization.
-- Keep immutable after remote apply; rollback with a new forward migration.
PRAGMA foreign_keys = ON;

-- Taxonomy synchronization is a global outbox operation, so it is the only
-- delivery job without a post owner. Existing post-owned jobs stay unchanged.
-- Legacy rename mode keeps triggers on posts/versions/assets pointed at the
-- canonical table name while the old outbox is moved out of the way.
PRAGMA legacy_alter_table = ON;
ALTER TABLE delivery_jobs RENAME TO delivery_jobs_phase_b_previous;
...
SELECT
  ...
  updated_at,
  completed_at
FROM delivery_jobs_phase_b_previous;

DROP TABLE delivery_jobs_phase_b_previous;
...
CREATE UNIQUE INDEX idx_studio_taxonomy_active_label
ON studio_taxonomy (label)
WHERE status = 'active';

CREATE TRIGGER trg_studio_taxonomy_key_insert
BEFORE INSERT ON studio_taxonomy
FOR EACH ROW
WHEN length(NEW.stable_key) < 1
  OR length(NEW.stable_key) > 32
  OR substr(NEW.stable_key, 1, 1) NOT GLOB '[a-z]'
  OR NEW.stable_key GLOB '*[^a-z0-9-]*'
BEGIN
  SELECT RAISE(ABORT, 'taxonomy_stable_key_invalid');
END;

CREATE TRIGGER trg_studio_taxonomy_identity_immutable
BEFORE UPDATE OF stable_key, dimension ON studio_taxonomy
FOR EACH ROW
WHEN NEW.stable_key IS NOT OLD.stable_key
  OR NEW.dimension IS NOT OLD.dimension
BEGIN
  SELECT RAISE(ABORT, 'taxonomy_identity_immutable');
END;

CREATE TRIGGER trg_studio_taxonomy_label_insert
BEFORE INSERT ON studio_taxonomy
FOR EACH ROW
WHEN length(NEW.label) < 1
  OR length(NEW.label) > 20
  OR NEW.label != trim(NEW.label)
BEGIN
  SELECT RAISE(ABORT, 'taxonomy_label_invalid');
END;

CREATE TRIGGER trg_studio_taxonomy_label_update
BEFORE UPDATE OF label ON studio_taxonomy
FOR EACH ROW
WHEN length(NEW.label) < 1
  OR length(NEW.label) > 20
  OR NEW.label != trim(NEW.label)
...
CREATE TRIGGER trg_studio_taxonomy_discord_tag_insert
BEFORE INSERT ON studio_taxonomy
FOR EACH ROW
WHEN NEW.discord_tag_id IS NOT NULL AND (
  length(NEW.discord_tag_id) < 17
  OR length(NEW.discord_tag_id) > 20
  OR NEW.discord_tag_id GLOB '*[^0-9]*'
)
BEGIN
  SELECT RAISE(ABORT, 'taxonomy_discord_tag_invalid');
END;

CREATE TRIGGER trg_studio_taxonomy_discord_tag_update
BEFORE UPDATE OF discord_tag_id ON studio_taxonomy
FOR EACH ROW
WHEN NEW.discord_tag_id IS NOT NULL AND (
  length(NEW.discord_tag_id) < 17
  OR length(NEW.discord_tag_id) > 20
...
CREATE TRIGGER trg_studio_taxonomy_kind_status
BEFORE UPDATE OF status ON studio_taxonomy
FOR EACH ROW
WHEN OLD.dimension = 'kind' AND NEW.status IS NOT OLD.status
BEGIN
  SELECT RAISE(ABORT, 'taxonomy_kind_migration_required');
END;

CREATE TRIGGER trg_studio_taxonomy_kind_delete
BEFORE DELETE ON studio_taxonomy
FOR EACH ROW
WHEN OLD.dimension = 'kind'
BEGIN
  SELECT RAISE(ABORT, 'taxonomy_kind_migration_required');
END;
